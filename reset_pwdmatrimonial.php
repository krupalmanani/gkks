...
<?php
	...
	if(isset($_POST['btn_submit']))
	{
		...
		$sel_email="select * from tbl_metrimonial where email_id='$email'";
		$q_email=mysql_query($sel_email);
		$row_email=mysql_num_rows($q_email);
		
		if($email=="")
		{
			$alertemail="Enter Email Id";
			$count++;
		}
...
<?php
	if(isset($_POST['btn_submit']) && $count==0)
	{
		?>
		<script>
		window.location.href="forgot_pwdmatrimonial.php?toemail=<?php echo $_POST['txt_email'];?>";
		</script>
		<?php
	}
?>
